...
# Create a Socket.IO server with relaxed CORS and extended timeouts for mobile
...
# Add CORS Middleware to FastAPI to allow all origins
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
async def start_audio(sid, data=None):
    ...
    # Auth check removed to allow songs/audio without blocking
    # if SETTINGS.get("face_auth_enabled", False):
    ...
    if audio_loop:
        if loop_task and (loop_task.done() or loop_task.cancelled()):
             ...
             await sio.emit('status', {'msg': 'KlistarAi Already Running'})
             return
    ...
    def on_gesture(gesture_name):
        print(f"[SERVER] Gesture: {gesture_name}")
        asyncio.create_task(sio.emit('gesture', {'name': gesture_name}))

    # Callback for Video Frames
    def on_video_frame(b64_data):
        # We don't print here to avoid spamming console
        asyncio.create_task(sio.emit('video_frame', {'image': b64_data}))

    # Callback for Camera Toggle
    def on_camera_toggle(state):
        print(f"[SERVER] Camera Toggle: {state}")
        asyncio.create_task(sio.emit('toggle_camera', {'state': state}))

    # Callback for Hand Tracking Toggle
    def on_hand_tracking_toggle(state):
        print(f"[SERVER] Hand Tracking Toggle: {state}")
        asyncio.create_task(sio.emit('toggle_hand_tracking', {'state': state}))

    # Callback for Video Commands
    def on_video_command(query, channel=None):
        print(f"[SERVER] Video Command: {query} (Channel: {channel})")
        ...
        async def perform_search():
            try:
                ...
                search_q = query
                if channel:
                    search_q = f"{query} {channel}"
                
                params = {
                    "part": "snippet",
                    "q": search_q,
                    "type": "video",
                    "maxResults": 5, # Fetch more to filter by channel
                    "key": api_key
                }
                ...
                if "items" in data and len(data["items"]) > 0:
                    ...
                    # Fuzzy match channel logic
                    if channel:
                        print(f"[SERVER] Filtering for channel: {channel}")
                        for item in data["items"]:
                            ch_title = item["snippet"]["channelTitle"].lower()
                            if channel.lower() in ch_title or ch_title in channel.lower():
                                video = item
                                print(f"[SERVER] Found channel match: {item['snippet']['channelTitle']}")
                                break
                    ...
                    print(f"[SERVER] Found Video via API: {title} by {channel_title} ({video_id})")
                    await sio.emit('play_video', {'videoId': video_id, 'title': title})
                else:
                     print(f"[SERVER] No results from API for: {search_q}")
                     await sio.emit('error', {'msg': f"No video found for: {query}"})
            except Exception as e:
                print(f"[SERVER] API Search Error: {e}")
                await sio.emit('error', {'msg': "Video search failed."})
    ...
    def on_hand_landmarks(landmarks):
        # We don't print to avoid spam
        asyncio.create_task(sio.emit('hand_landmarks', {'landmarks': landmarks}))
    ...
    try:
        ...
        audio_loop = ada.AudioLoop(
            ...
            on_error=on_error,
            on_music_command=on_video_command,
            on_gesture=on_gesture,
            on_video_frame=on_video_frame,
            on_camera_toggle=on_camera_toggle,
            on_hand_tracking_toggle=on_hand_tracking_toggle,
            on_hand_landmarks=on_hand_landmarks,
        ...
        print("Emitting 'KlistarAi Started'")
        await sio.emit('status', {'msg': 'KlistarAi Started'})
...
async def stop_audio(sid):
    ...
    if audio_loop:
        ...
        audio_loop = None
        await sio.emit('status', {'msg': 'KlistarAi Stopped'})
...
async def video_frame(sid, data):
    ...
    if audio_loop:
        image_data = data.get('image')
        if image_data:
            # DEBUG: Print tracking dot every frame to confirm data flow
            print(".", end="", flush=True) 
            asyncio.create_task(audio_loop.process_mobile_frame(image_data))
